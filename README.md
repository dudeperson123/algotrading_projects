...
3. symbolscorer.py:
     1. Run nasdaq_generator.py (This will create a folder of all historically available 1D data for all nasdaq symbols from yfinance)
     2. Run symbolscorer.py (This program will create scores for symbols with indicators like RSI, MFI, %K, etc. It then backtests a strategy that trades symbols with the highest scroes, and prints a backtest summary with metrics at the end.)
  
4. gptpapertrader.py:
     1. Set up OpenAI keys (see OpenAI API Documentation at https://developers.openai.com/api/docs)
     2. Make sure they are set as environment variables and billing info is set up
     3. Run gptpapertrader.py (this program outputs a live log of gpt-5-mini's decisions trading BTC/USD based on its past OHLCV data. It also logs to a csv, allowing reconstruction of trades, backtests, and evaluation metrics.)
